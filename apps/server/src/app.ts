...
export function createServer(eventSink?: WorkspaceEventSink, mcpToken?: string, assetProvider?: AssetProvider, dataDirectory?: string): FastifyInstance {
  const app = Fastify({ logger: false });
  app.get("/health", async () => ({ ok: true, phase: 2 }));
  const registry = registerWorkspaceRoutes(app, new WorkspaceRegistry(eventSink, dataDirectory));
